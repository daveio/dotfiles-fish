...
function quickcommit -d "Quick git workflow: add all, commit with AI, optionally push"
    if test (count $argv) -eq 0
        # Default behavior: add, commit with AI
        git add -A .
        oco --fgm --yes
    else if test "$argv[1]" = "--push" -o "$argv[1]" = "-p"
        # Add, commit, and push
        git add -A .
        oco --fgm --yes
        git push
        ...
        git add -A .
        git commit -m "$argv[1]"
        if test (count $argv) -gt 1 -a "$argv[2]" = "--push" -o "$argv[2]" = "-p"
            git push
        end
    end
end

function trunkfix -d "Run trunk format and check in sequence"
    ...
    if test $status -eq 0
        ...
        echo "❌ Trunk format failed"
        return 1
    end
end

function devsetup -d "Quick development setup"
...
function gitclean -d "Clean up git repository"
...
function dockerclean -d "Clean up Docker system"
